...
def combine_dbcan():
    dbcan = polars.read_parquet("src/data/genecat_output/dbcan.features.parquet")
    pfam = polars.read_parquet("src/data/genecat_output/pfam.features.parquet")

    joined = pfam.vstack(dbcan).sort(by=["sequence_id", "domain_start"])
    joined.write_parquet("src/data/genecat_output/dbcan.pfam.features.parquet")
