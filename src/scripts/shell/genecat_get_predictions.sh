...
FINETUNED_MODEL=$(ls -t "${OUT_PFAM}/models_fold_${SLURM_ARRAY_TASK_ID}"/*.pt 2>/dev/null | head -n 1)
...
python -m genecat.cli pul-finetune\
 ...
 --middle-focus --epochs 30 --predict-with-model $FINETUNED_MODEL
...
MODEL=${BASEPATH}/models_multilabel_models/april_models/${MODEL_NAME}
FINETUNED_MODEL=$(ls -t "${OUT_CAZY}/models_fold_${SLURM_ARRAY_TASK_ID}"/*.pt 2>/dev/null | head -n 1)
# DOMAINS
